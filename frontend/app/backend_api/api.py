...
async def login_user(user_email: str, password: str):
    async with httpx.AsyncClient() as client:
        response = await client.post(
            url=f"{settings.BACKEND_API}auth/login", data={"username": user_email, "password": password}
        )
...
async def get_user_info(access_token: str):
    async with httpx.AsyncClient() as client:
        response = await client.get(
            url=f"{settings.BACKEND_API}auth/get-my-info", headers={"Authorization": f"Bearer {access_token}"}
        )
        return response.json()
...
async def get_products(q: str = ""):
    async with httpx.AsyncClient() as client:
        response = await client.get(url=f"{settings.BACKEND_API}products/", params={"q": q})
        return response.json()
